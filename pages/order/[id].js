...
import { FlutterwaveButton, Flutterwave } from 'flutterwave-react-v3';
...
})
{
  ...
  const config = {
  public_key: flutterwave_public_key, // your Flutterwave public key
  tx_ref: Date.now().toString(), // unique transaction reference
  amount: orderData.total, // amount to charge
          ...
          <div className={styles.order__actions}>
            ...
            {!orderData.isPaid && (
              <div className={styles.order__payment}>
                ...
                {orderData.paymentMethod == "mobileMoney" && (
                  <button
        onClick={() => {
...
export async function getServerSideProps(context) {
  ...
  let flutterwave_public_key = process.env.FLUTTERWAVE_PUBLIC_KEY;
  let mtn_public_key = process.env.MTN_PUBLIC_KEY;
  let mtn_secret_key = process.env.MTN_SECRET_KEY;
  db.disconnectDb();
  return {
    props: {
